...
- 2026-08-31 14:52 (このセッション，MATUTOSI_DP) その6
  **表題帯を acposter に揃え，`poster_howto.qmd` を acposter と同じ箱立てにした** (ユーザ指示)．
  - **表題帯**: 中央揃えにし，著者と所属を**「氏名(所属)」の1行**にまとめた
    ...
    **配列で渡す**ように変えた．acposter の `make_byline` と同じ規則)．
  - **注記 (`note`) を表題帯の中へ移した** (`title-box` の `keywords:` 枠を借りる)．
    ページ下端の帯 (`pop.bottom-box`) は**やめた**．acposter・ggposter と見た目が揃い，
    紙面が詰まっても溢れない．
  - **`poster_howto.qmd` を acposter の `poster_howto.md` と同じ箱立て・同じ順**にした
    (2段組み・9箱)．コードの箱と姉妹ツールとの行き来の箱を落とし，写真の横並びを
    `{.full}` の全幅の箱にして3枚並べた．`{.break}` は qtposter 固有なので残した．
  - **1ページに収めるのに寸法を詰めた**: `font-size` 30→26pt，`fig-max-height: 12%`，
    写真は `{width=40%}`，救済の図は `{width=45%}`．
    写真を 55% のままにすると**2ページに溢れる** (実測)．
  - **検証**: 見本5本とも A0・1ページ．縮小画像4枚を作り直した．

- 2026-08-31 12:30 (このセッション，MATUTOSI_DP) その5
  **技術的な残り3件を実装した** (ユーザ指示)．これで acposter との機能差は無くなった．
  - **`{.full}` (全幅の箱)**: 段組みはテンプレートではなく**フィルタが塊ごとに掛ける**
    形に変え，`{.full}` の箱の前で段組みを閉じ，あとで開き直す．
    **段組みの塊は塊ごとに均等割りされる**ので，全幅の箱を中ほどに置くと縦が余る．
    見本では最後の箱に付けた．
  - **図の高さの上限**: `layout()` が返す「その箱に使える高さ」の割合で決める
    (既定 25%)．超える画像は `scale` で比を保って縮む．`fig-max-height:` で変えられる．
    縦長の画像で発火することを実機で確認した．
  - **表の体裁**: booktabs 調 (上端・見出しの下・下端の3本だけ．縦罫なし)．
  - **見本4本＋最小の見本はすべて1ページ**．`golf_course.qmd` は表と写真が多く，
    18pt まで下げて収めた．
  - README から**他プロジェクトの記載を落とした** (見本・現状と経緯・ライセンス)．
    ユーザ指示．姉妹ツールの話は「姉妹ツールとの行き来」に集約する．
...
- **【点検 2026-08-31】acposter と比べて足りないもの** (実際に組んで確かめた．
  ...
  2. ~~`{.full}` (全幅の箱)~~ → **2026-08-31 に足した** (段組みを閉じて全幅で置き，開き直す)．
  3. ~~画像の救済が無い~~ → **2026-08-31 に足した** (`Link` フィルタ．acposter と同じ約束)．
  4. ~~画像の高さの上限~~ → **2026-08-31 に足した** (箱に使える高さの 25%．`fig-max-height:` で変更可)．
  5. ~~検算が無い~~ → **2026-08-31 に `check_poster_pdf.ps1` を足した**
     (ページ数・用紙実寸・埋め込みフォント．出力の文言は acposter に揃えた)．
  6. ~~表の体裁が pandoc 既定の全罫線~~ → **2026-08-31 に booktabs 調にした**．
